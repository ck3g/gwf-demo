...
func TestUser_GetByEmail(t *testing.T) {
	u, err := models.Users.GetByEmail("john.doe@example.com")
	if err != nil {
		t.Error("failed to get user: ", err)
	}
...
func TestUser_Update(t *testing.T) {
	...
	if err != nil {
		t.Error("failed to uopdate users: ", err)
	}
...
func TestUser_PasswordMatches(t *testing.T) {
	...
	matches, err = u.PasswordMatches("invalid-password")
	if err != nil {
		t.Error("error checking match: ", err)
	}
...
func TestUser_ResetPassword(t *testing.T) {
	...
	if err == nil {
		t.Error("did not get an error when trying to reset a password for non-existed user")
	}
}
...
func TestToken_GetUserForToken(t *testing.T) {
	...
	if err == nil {
		t.Error("error expected by not received when getting user with a bad token")
	}
...
var authData = []struct {
	...
	{"invalid", "abcdefghijklmnopqrstuvwxyz", "a@example.com", true, "invalid token accepted as valid"},
	{"invalid_length", "abcdefghijklmno", "a@example.com", true, "token of wrong length accepted as valid"},
	{"no_user", "abcdefghijklmnopqrstuvwxyz", "a@example.com", true, "no user, but token accepted as valid"},
	{"valid", "", "john.doe@example.com", false, "valid token reported as invalid"},
}
...
func TestToken_ValidToken(t *testing.T) {
	...
	ok, err = models.Tokens.ValidToken("invalid-token")
	if ok {
		t.Error("valid token reported as valid")
	}
	...
	if ok {
		t.Error("no error reported when validatation non-existent token")
	}
}
